...
int sumDigits(int n) {
	...
	int placing = ((to_string(n)).length()) - 1;
	int div = power(10.0, placing);
	int highest_val = (n/div) * div;
	if(n < 10){
		return n;
	}
	return (n/div) + sumDigits(n - highest_val);
}
...
int countOccurrences(const int* arr, int n, int x) {
	...
	return i + countOccurrences(arr, n-1, x); // fix this!
}
